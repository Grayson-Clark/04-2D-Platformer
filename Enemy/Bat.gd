...
func _physics_process(_delta):
	...
	if player == null:
		player = get_node_or_null("/root/SceneHolder/Game/Player_Container/Player")
	else:
		...
		if c and c.name == "Player":
			line_of_sight = true
		points = get_node("/root/SceneHolder/Game/Navigation2D").get_simple_path(get_global_position(), player.global_position, true)
		if points.size() > 1:
